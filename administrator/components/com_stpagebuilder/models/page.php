 ...
 * @subpackage  com_book
 */
class StpagebuilderModelPage extends JModelAdmin
{
	private $code;

	public function getTable($type = 'Page', $prefix = 'StpagebuilderTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_stpagebuilder.page', 'page', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	public function savePage($data,$id,$name,$config){
		...
		if($kt){
			return JText::_("COM_STPAGEBUILDER_SUCCESS_SAVE");
		}else{
			echo "loi roi";
		}
	}
	private function editPage($id,$name,$config){
		...
		$where  = array('id='.$id);
		$query->update($db->quoteName('#__stpagebuilder'))->set($field)->where($where);
	...
	public function stdelete(){
		...
		foreach ($ids as $key => $id) {
			$query= $db->getQuery(true);
			$query->delete($db->quoteName('#__stpagebuilder'))->where('id='.$id);
			$db->setQuery($query);
			$this->stexcute($db);
			$query= $db->getQuery(true);
			$query->delete($db->quoteName('#__stpagebuilder_design'))->where('pageid='.$id);
			$db->setQuery($query);
			$this->stexcute($db);
		}
		return true;
	...
	public function getDesign($id){
		...
		$query 		->select('a.*,b.class,b.name')
					->from('#__stpagebuilder_design AS a')
					->join('INNER', '#__stpagebuilder_type AS b ON (a.type = b.type)')
					->where('pageid='.$id)
					->order('a.id ASC');
		$db->setQuery($query);
	...
	public function getItemcode(){
		$db 	= $this->getDbo();
		$query  = $db->getQuery(true);
		$query->select('*')->from('#__stpagebuilder_type');
		$db->setQuery($query);
		return $db->loadObjectList();
	}
	...
	public function getPageName($id){
		$db 	= $this->getDbo();
		$query  = $db->getQuery(true);
		$query->select('name')->from('#__stpagebuilder')->where('id='.$id);
		$db->setQuery($query);
		return $db->loadResult();
	}
	...
	public function addNewPage($name){
		...
		$query 	->insert($db->quoteName('#__stpagebuilder'))
	...
	private function AddDesign($id,$data){
		...
		foreach($data as $item){
			...
			$query 	->insert($db->quoteName('#__stpagebuilder_design'))
			    	->columns($db->quoteName($columns))
			    	->values(implode(',', $values));
	    	$db->setQuery($query);
	    	try {
	...
	public function duplicate($id){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('*')->from('#__stpagebuilder')->where('id='.$id);
		$db->setQuery($query);
		$page = $db->loadObject();
		...
		$query = $db->getQuery(true);
		$query->select('id')->from('#__stpagebuilder_design')->where('pageid='.$id)->order('pageid ASC');
		$db->setQuery($query);
	...
	private function duplicateDesign($id,$newid){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('*')->from('#__stpagebuilder_design')->where('id='.$id);
		$db->setQuery($query);
		$desi = $db->loadObject();
